...
int aes_init(unsigned char *key_data, int key_data_len, unsigned char *salt)
{
  ...
  e_ctx = malloc(sizeof(EVP_CIPHER_CTX)+1);
  d_ctx = malloc(sizeof(EVP_CIPHER_CTX)+1);
  /*
 ...
 */
 char *aes_encrypt(unsigned char *plaintext, int *len){
  ...
  if(!EVP_EncryptInit_ex(e_ctx, NULL, NULL, NULL, NULL)){
    perror("ERROR in EVP_EncryptInit_ex \n");
    return NULL;
  }
 
  /* update ciphertext, c_len is filled with the length of ciphertext generated,
    *len is the size of plaintext in bytes */
  if(!EVP_EncryptUpdate(e_ctx, ciphertext, &c_len, plaintext, *len)){
    perror("ERROR in EVP_EncryptUpdate \n");
    return NULL;
  }
 
  /* update ciphertext with the final remaining bytes */
  if(!EVP_EncryptFinal_ex(e_ctx, ciphertext+c_len, &f_len)){
    perror("ERROR in EVP_EncryptFinal_ex \n");
    return NULL;
  }
 ...
 char *aes_decrypt(unsigned char *ciphertext, int *len)
{
  ...
  if(!EVP_DecryptInit_ex(d_ctx, NULL, NULL, NULL, NULL)){
    perror("ERROR in EVP_DecryptInit_ex \n");
    return NULL;
  }
 
  if(!EVP_DecryptUpdate(d_ctx, plaintext, &p_len, ciphertext, *len)){
    perror("ERROR in EVP_DecryptUpdate\n");
    return NULL;
  }
 
  if(!EVP_DecryptFinal_ex(d_ctx, plaintext+p_len, &f_len)){
    perror("ERROR in EVP_DecryptFinal_ex\n");
    return NULL;
  }
